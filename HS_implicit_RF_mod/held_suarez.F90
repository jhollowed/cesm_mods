...
!======================================================================= 
  ...
  subroutine held_suarez_1994(pcols, ncol, clat, pmid, ptop,&
       u, v, t, du, dv, s)
    ...
    real(r8), intent(in)  :: pmid(pcols,pver) ! mid-point pressure in Pa
    ! CJ add
    real(r8), intent(in)  :: ptop             ! interface pressure at the model top (constant) in Pa
...
!   real(r8), parameter :: fv3_rf_cutoff = 0.00001_r8  ! 1 Pa in sigma coordinate, Rayleigh friction above it
    real(r8), parameter :: fv3_rf_cutoff = 0.001_r8    ! 100 Pa in sigma coordinate, Rayleigh friction above it
    real(r8), parameter :: fv3_tau_rev   = 1._r8/(86400._r8*1._r8)  ! 1/(1 day) damping time scale 
    !
    !-----------------------------------------------------------------------
    !
    ! Initialize pi/2
    ...
    do k = 1, pver
      ...
      if (pref_mid_norm(k) < fv3_rf_cutoff) then   ! RF above the cutoff pressure
         kr = fv3_tau_rev * (sin(pih*log(fv3_rf_cutoff/pref_mid_norm(k))/log(fv3_rf_cutoff/ptop)))**2
         do i = 1, ncol
           du(i,k) = -kr*u(i,k)   ! tendency
           dv(i,k) = -kr*v(i,k)   ! tendency 
         end do
       endif
    end do
