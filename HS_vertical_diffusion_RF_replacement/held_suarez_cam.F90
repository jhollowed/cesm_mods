...
! CJ rair, gravit added
    use physconst,          only: cappa, cpair, rair, gravit
...
! CJ rair, gravit added
    call held_suarez_1994_init(cappa, cpair, rair, gravit, psurf_ref, pref_mid_norm)
...
!
!   CJ modified call to routine to enable vertical diffusion
    call held_suarez_1994(pcols, ncol, clat, ztodt, state%pmid, state%pint, state%rpdel, &
         state%u, state%v, state%t, ptend%u, ptend%v, ptend%s)
